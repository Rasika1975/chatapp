...
// Add context menu listeners to message area
function addContextMenuListeners() {
    ...
    // Add right-click listener to the message area
...
// Update displayMessage function to handle images and deletions
function displayMessage(msg, showNotif) {
    ...
    messageDiv.dataset.messageId = msg.id; // Store message ID for deletion
    ...
    let isDeleted = false;
    if (isSent) {
        // For sender: message is deleted if it's deleted for sender OR if it's deleted for everyone
        isDeleted = msg.deletedForSender || msg.deletedStatus;
    } else {
        // For receiver: message is deleted if it's deleted for receiver OR if it's deleted for everyone
        isDeleted = msg.deletedForReceiver || msg.deletedStatus;
    }
    
    // 🔥 IMAGE HANDLING
    let messageContent = '';
    if (isDeleted) {
        messageContent = '<span style="font-style: italic; opacity: 0.6;">This message was deleted</span>';
    } else {
    ...
    // Add delete options for sent messages
    if (isSent && !isDeleted) {
        messageHtml += `
        <div class="dropdown" onmouseleave="hideDropdownDelayed(this)">
            <button class="dropbtn" onclick="toggleDropdown(this)">▼</button>
            <div class="dropdown-content" onmouseenter="cancelHideDropdown()" onmouseleave="hideDropdown(this)">
                <a href="#" onclick="event.preventDefault(); deleteMessageForMe(${msg.id});">Delete for me</a>
                <a href="#" onclick="event.preventDefault(); deleteMessageForEveryone(${msg.id});">Delete for everyone</a>
            </div>
        </div>`;
    }
    ...
    // Notification
    if (!isSent && showNotif && msg.senderId != currentUserId && !isDeleted) {
...
function showDeleteOptions(messageId) {
    console.log('🗑️ Showing delete options for message:', messageId);
    
    const options = [
        { label: 'Delete for me', action: 'deleteforme' },
        { label: 'Delete for everyone', action: 'deleteforeveryone' }
    ];
    
    const choice = prompt(`Choose delete option:
1. ${options[0].label}
2. ${options[1].label}
...
// 🆕 NEW - Delete message for sender only
async function deleteMessageForMe(messageId) {
    try {
        const response = await fetch(`${API_URL}/messages/deleteforme/${messageId}/${currentUserId}`, {
            method: 'DELETE'
        });
        
        const result = await response.text();
        console.log('🗑️ Delete for me result:', result);
        
        // Update the message in the UI
        updateMessageInUI(messageId, 'deletedforme');
...
async function deleteMessageForEveryone(messageId) {
    try {
        const response = await fetch(`${API_URL}/messages/deleteforeveryone/${messageId}/${currentUserId}`, {
            method: 'DELETE'
        });
        
        const result = await response.text();
        console.log('🗑️ Delete for everyone result:', result);
        
        // Update the message in the UI
        updateMessageInUI(messageId, 'deleteforeveryone');
...
// 🆕 NEW - Update message in UI after deletion
function updateMessageInUI(messageId, action) {
    console.log('🔄 Updating message in UI:', messageId, action);
    
    // Find all message elements with this ID
    const messageElements = document.querySelectorAll(`[data-message-id="${messageId}"]`);
    
    messageElements.forEach(element => {
        // Update the content to show deletion
        const contentDiv = element.querySelector('.message-content');
        if (contentDiv) {
            contentDiv.innerHTML = '<span style="font-style: italic; opacity: 0.6;">This message was deleted</span>';
...
// Dropdown menu functions for better UX
...
function hideDropdownDelayed(dropdown) {
    const content = dropdown.querySelector('.dropdown-content');
...
// Alternative delete functions for context menu
function deleteMessageForMe(messageId) {
    ...
    fetch(`${API_URL}/messages/deleteforme/${messageId}/${currentUserId}`, {
        method: 'DELETE'
    })
...
function deleteMessageForEveryone(messageId) {
    ...
    fetch(`${API_URL}/messages/deleteforeveryone/${messageId}/${currentUserId}`, {
        method: 'DELETE'
    })
    .then(response => response.text())
    .then(result => {
